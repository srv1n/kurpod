...
impl SessionManager {
    ...
    pub fn create_session(
        ...
        self.create_bearer_token(&session_id, &client_key_part, client_ip.clone(), user_agent.clone())
    }
    ...
    fn create_bearer_token(
        ...
        let token_json = serde_json::to_string(&token)
            .map_err(|_| "Failed to serialize token")?;
    ...
    pub fn validate_token(
        ...
        let mut mac = HmacSha256::new_from_slice(&self.secret_key)
            .map_err(|_| "Invalid HMAC key")?;
        mac.update(token_b64.as_bytes());
        let expected_signature = base64::prelude::BASE64_STANDARD.encode(mac.finalize().into_bytes());
        ...
        // Decode token
        let token_json = base64::prelude::BASE64_STANDARD.decode(token_b64)
            .map_err(|_| "Invalid token encoding")?;
        let token: SessionToken = serde_json::from_slice(&token_json)
...
mod tests {
    ...
    async fn test_session_manager() {
        ...
        // Validate token
        let (session_id, client_key_part) = manager.validate_token(
            &token,
            client_ip,
            user_agent,
        ).unwrap();
