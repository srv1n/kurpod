...
const FileViewer = ({ 
    ...
    const handleTouchMove = useCallback((e) => {
        ...
        const isDismissing = deltaY > 30 && isMobile; // Only downward swipes on mobile
    ...
    }, [swipeToClose.startY, isMobile]);

    const handleTouchEnd = useCallback(() => {
        const verticalDistance = swipeToClose.currentY - swipeToClose.startY;
        const horizontalDistance = touchStart && touchEnd ? touchStart - touchEnd : 0;
        
        // Check for vertical dismiss gesture (downward swipe)
        if (verticalDistance > dismissThreshold && swipeToClose.isDismissing && isMobile) {
            onClose();
            setSwipeToClose({ startY: 0, currentY: 0, isDismissing: false });
            return;
        }
        
        // Handle horizontal navigation only if not dismissing and we have touch data
        if (!swipeToClose.isDismissing && touchStart && touchEnd) {
        ...
        // Reset swipe state
        setSwipeToClose({ startY: 0, currentY: 0, isDismissing: false });
    }, [touchStart, touchEnd, hasNext, hasPrevious, navigateNext, navigatePrevious, swipeToClose, dismissThreshold, onClose, isMobile]);
    ...
    return (
        ...
        >
            ...
            {!isFullscreen && (
                <div className="bg-black/90 text-white relative z-30">
                ...
                <div className="absolute inset-0 pointer-events-none">
                    ...
                    <div className="absolute top-4 right-4 pointer-events-auto opacity-0 hover:opacity-100 transition-opacity duration-300">
                        <div className="flex items-center space-x-2">
                            <button
                                onClick={toggleFullscreen}
                                className="p-3 rounded-lg bg-black/50 text-white hover:bg-black/70"
                            >
                    ...
                    <div className="absolute inset-y-0 left-4 flex items-center pointer-events-auto">
                        <button
                            onClick={navigatePrevious}
                            disabled={!hasPrevious}
                            className="p-4 rounded-lg bg-black/50 text-white hover:bg-black/70 disabled:opacity-50 disabled:cursor-not-allowed opacity-0 hover:opacity-100 transition-opacity duration-300"
                        >
                            <ChevronLeftIcon className="w-8 h-8" />
                        </button>
                    </div>
                    ...
                    {/* Bottom info */}
                    <div className="absolute bottom-4 left-4 pointer-events-auto opacity-0 hover:opacity-100 transition-opacity duration-300">
