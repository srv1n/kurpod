...
// Get file size without reading entire file (Mobile Only)
// Note: This should not be needed on iOS since frontend can get file size directly
// But keeping for compatibility with the streaming import flow
#[cfg(any(target_os = "ios", target_os = "android"))]
#[tauri::command]
fn get_file_size(file_path: String) -> ApiResponse<u64> {
    println!("[get_file_size] Getting size for: {}", file_path);
    
    // For iOS, the frontend should handle file size detection using @tauri-apps/plugin-fs
    // This backend function is mainly for compatibility but may not work with iOS security scoping
    match fs::metadata(&file_path) {
        Ok(metadata) => {
            let size = metadata.len();
            println!("[get_file_size] File size: {} bytes", size);
            ApiResponse {
                success: true,
                data: Some(size),
                message: None,
        ...
        Err(e) => {
            println!("[get_file_size] Failed to get file metadata: {}", e);
            // For iOS, suggest using frontend file size detection instead
            ApiResponse {
                success: false,
                data: None,
                message: Some(format!("Failed to get file size: {}. On iOS, use frontend file size detection instead.", e)),
            }
        }
    }
}

// Stream-based chunk reading (Mobile Only)
#[cfg(any(target_os = "ios", target_os = "android"))]
#[tauri::command]
fn read_file_chunk(file_path: String, offset: u64, chunk_size: usize) -> ApiResponse<Vec<u8>> {
    println!("[read_file_chunk] Reading {} bytes from offset {} in {}", chunk_size, offset, file_path);
    
    match fs::File::open(&file_path) {
        Ok(mut file) => {
            // Seek to the specified offset
            if let Err(e) = file.seek(SeekFrom::Start(offset)) {
                println!("[read_file_chunk] Failed to seek to offset {}: {}", offset, e);
                return ApiResponse {
                    success: false,
                    data: None,
                    message: Some(format!("Failed to seek in file: {}", e)),
                };
            }
            ...
            match file.read(&mut buffer) {
                Ok(bytes_read) => {
                    ...
                    println!("[read_file_chunk] Successfully read {} bytes", bytes_read);
                    ApiResponse {
                        success: true,
                        data: Some(buffer),
                        message: None,
                ...
                Err(e) => {
                    println!("[read_file_chunk] Failed to read chunk: {}", e);
                    ApiResponse {
                        success: false,
                        data: None,
                        message: Some(format!("Failed to read file chunk: {}", e)),
                    }
                }
            }
        },
        Err(e) => {
            println!("[read_file_chunk] Failed to open file: {}", e);
            ApiResponse {
                success: false,
                data: None,
...
// Desktop fallbacks for non-mobile platforms
#[cfg(not(any(target_os = "ios", target_os = "android")))]
#[tauri::command]
fn get_file_size(_file_path: String) -> ApiResponse<u64> {
    return ApiResponse {
        success: false,
        data: None,
        message: Some(format!("Get file size not supported on non-mobile platforms")),
    };
}

#[cfg(not(any(target_os = "ios", target_os = "android")))]
#[tauri::command]
fn read_file_chunk(_file_path: String, _offset: u64, _chunk_size: usize) -> ApiResponse<Vec<u8>> {
    return ApiResponse {
        success: false,
        data: None,
        message: Some(format!("Read file chunk not supported on non-mobile platforms")),
    };
}
