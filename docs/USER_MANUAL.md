# ENC Server User Manual

A comprehensive guide to using ENC Server for secure encrypted file storage.
...
ENC Server is a secure file storage system that encrypts all your files in a single blob file while providing an easy-to-use web interface. It's designed for users who need:
...
1. Go to the [Releases page](https://github.com/yourusername/enc-server/releases)
2. Download the appropriate file for your system:
   - **Linux**: `enc_server-vX.X.X-linux-amd64.tar.gz`
   - **macOS (Apple Silicon/M1/M2)**: `enc_server-vX.X.X-darwin-arm64.tar.gz`
   - **macOS (Intel)**: `enc_server-vX.X.X-darwin-amd64.tar.gz`
...
2. Navigate to where you extracted the `enc_server` binary
3. Run the server:
   ```bash
   ./enc_server
   ```
...
```
2025-02-06T10:30:00.000Z INFO  Starting ENC Server...
...
ENC Server supports two independent storage volumes:
...
When you start ENC Server and visit the web interface:
...
```
┌─────────────────────────────────────────────────────────┐
│ ENC Server - [Standard Volume] or [Hidden Volume]      │
├─────────────────────────────────────────────────────────┤
...
2. Drag them into the ENC Server window
...
- ENC Server is accessible to all devices on your local network
...
Create a systemd service file `/etc/systemd/system/enc-server.service`:

```ini
[Unit]
Description=ENC Server
...
sudo systemctl enable enc-server
...
1. **File Types**:
   - ENC Server works with all file types
